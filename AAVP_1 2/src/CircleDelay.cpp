...
double CircleDelay::process(double input,int transpose, bool key, int cutoffSlider, int resSlider)
{
    ...
    //int SampleRate = 44100;
    //float filtered = biquad.process(, cutoffSlider, resSlider, slider);
    
    Buffer[BufferIndex] = biquad.process (input + (Buffer[BufferIndex] * Feedback), cutoffSlider, resSlider);
    
    BufferIndex ++;
    BufferIndex%=calcNote(transpose, key);
    return output;
}
